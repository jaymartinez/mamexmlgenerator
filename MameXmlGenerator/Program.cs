    ...
    class Program
    {
        static void Main(string[] args)
        {
            if (args?.Length > 0)
            {
                ...
                XmlReader xmlReader = XmlReader.Create($"{path}\\MAME-large.xml");

                //List<mameMachine> mameMachineList = new List<mameMachine>();
                XmlWriterSettings settings = new XmlWriterSettings();
                settings.Indent = true;
                settings.IndentChars = ("\t");
                XmlWriter writer = XmlWriter.Create($"{path}\\MAME-haveRoms.xml", settings);
